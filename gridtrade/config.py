...
DEFAULT_TIER_POLICY = TierPolicy(
    ...
    # 上市巡检再补。
    tier1=(),
    tier2_cap=1,   # 同币开仓上限(2026-07-15 用户定 2→1;强制每币≤1格,更分散;lev_caps 叠加取更严)
)
...
DEFAULT_STRATEGY_CONFIG = {
    ...
    'grid_v2_config': {
        ...
        # 网格加密(2026-07-15 币安三轮坐标下降+留出窗验收,候选A;spec 2026-07-15-binance-param-sweep):
        # cmin 10→20 是三轮里唯一收敛的维度(geom 族 BASE 稳居榜首),留出窗守住(A +6.6% vs 现值
        # +5.4%,两窗逐个胜)。band/spacing 在同扫描里 1.5↔2/0.00875↔0.03 震荡=噪声,保持现值不动。
        'grid_count_min': 20,          # ←10:密网,币安小市值币波动大、密网抓更多回调成交
        'grid_count_max': 149,
...
DEFAULT_STOP_CFG = {
    # 固定止损收紧(2026-07-15 币安三轮坐标下降,候选A;取代 2026-07-10 HL 口径的 0.045):
    # 加了 MDD 维度后止损结构清晰——收益对止损几乎中性(八档 ±1.4pp),只有最大回撤单调响应:
    # 0.035→最差窗 MDD −1.4%、0.045→−1.9%、OFF→−3.0%。故按回撤容忍度定,收紧到 0.035 压 MDD
    # 而收益不减。留出窗验收 A 守住(样本外 +6.6% vs 现值 +5.4%)。丝(破网价挂单)与本参数无关、不动。
    'stop_loss': 0.035,
    # 连续回撤止盈关闭(2026-07-15 币安三轮坐标下降,候选A;trailing_k=None 使 stop_rules/引擎
    # 跳过整块——见 stop_rules.py 的 `if k is not None and floor is not None`):三轮里 trailing OFF
    # 都在顶部,关掉四窗更优。⚠权衡:关掉会失去趋势崩盘窗的回撤保险——留出窗 HOLD-A(2025-02~03
    # 币种中位 −46% 腰斩)实测,A(关)+2.55%/MDD−3.66% vs 现值(开)+1.43%/MDD−2.98%:关掉收益更高
    # 但回撤更深 ~0.7pp。零破网零爆仓、可接受;若更重回撤可单独保留 trailing(见 spec 备选)。
    'trailing_k': None,
    'trailing_floor': None,
