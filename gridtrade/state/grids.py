...
class GridRepository:
    ...
    def set_close_reason(self, grid_id, reason) -> None:
        # 关格真因落库（close() 发起时）：进程死在关格中途，续平方仍能还原原始触发原因。
        with self.engine.begin() as c:
            c.execute(update(grids).where(grids.c.id == grid_id)
                      .values(close_reason=reason, updated_at=now_ms()))
