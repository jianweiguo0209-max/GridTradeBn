...
_BASELINE_OVERRIDE = {}      # Pass 2（坐标下降）用：把基线换成 Pass 1 各族冠军的组合
...
def baseline():
    """当前扫描基线 = 实盘现值 + 覆盖项（Pass 1 恒为实盘现值；Pass 2 换成组合候选）。"""
    return dict(live_baseline(), **_BASELINE_OVERRIDE)


def set_baseline(overrides):
    """设置基线覆盖（Pass 2 坐标下降）。

    **产物必须分目录**：CSV 的臂标签只编码「被扫维度」的值，其余维度取自基线——同名臂在
    不同基线下含义不同，混在一个 CSV 里会串味（例：Pass1 的 'mult=5' 是「其余全旧值」，
    Pass2 的 'mult=5' 是「其余全新值」）。CLI 用 --out 强制隔离。"""
    global _BASELINE_OVERRIDE
    unknown = set(overrides) - set(live_baseline())
    if unknown:
        raise ValueError('未知基线维度: %s' % sorted(unknown))
    _BASELINE_OVERRIDE = dict(overrides)
    return baseline()
...
def build_arms(family):
    """本族的臂网格。第一臂恒为 BASE（=当前基线，Pass 1 是实盘现值、Pass 2 是组合候选）；
    与基线等价的臂自动去重（Pass 2 换基线后，网格里必有一点落在新基线上）。"""
    b = baseline()
    cand = []
    if family == 'stop':
        for sl in (0.030, 0.035, 0.040, 0.045, 0.055, 0.065, 0.080):
            cand.append(Arm(family, 'sl=%.3f' % sl, {'stop_loss': sl}))
        cand.append(Arm(family, 'sl=OFF', {'stop_loss': 9.9}))     # 9.9=事实停用（仅破网/爆仓兜底）
    elif family == 'trail':
        for k in (0.10, 0.15, 0.25):
            for fl in (0.010, 0.015, 0.025):
                cand.append(Arm(family, 'k=%.2f,fl=%.3f' % (k, fl),
                                {'trailing_k': k, 'trailing_floor': fl}))
        cand.append(Arm(family, 'trail=OFF', {'trailing_k': 9.9, 'trailing_floor': 9.9}))
    elif family == 'pv':
        for thr in (0.0, 0.0025, 0.005, 0.0075, 0.010):
            cand.append(Arm(family, 'thr=%.4f' % thr, {'pv_thr': thr}))
        for mult in (2, 3, 4):
            cand.append(Arm(family, 'mult=%d' % mult, {'pv_mult': mult}))
            for thr in (0.0025, 0.0075):
                cand.append(Arm(family, 'thr=%.4f,mult=%d' % (thr, mult),
                                {'pv_thr': thr, 'pv_mult': mult}))
        cand.append(Arm(family, 'pv=OFF', {'active_stop_mode': 'none'}))
    elif family == 'funding':
        for f in (0.0005, 0.0010, 0.0015, 0.0025):
            cand.append(Arm(family, 'fr=%.4f' % f, {'funding_stop': f}))
        cand.append(Arm(family, 'fr=OFF', {'funding_stop': 9.9}))
    elif family == 'geom':
        for band in (1.5, 2, 3, 4, 5):
            for cmin in (5, 10, 20, 30):
                cand.append(Arm(family, 'band=%s,cmin=%d' % (_num(band), cmin),
                                {'band': band, 'count_min': cmin}))
    elif family == 'spacing':
        for sm in (0.02, 0.03, 0.04, 0.06, 0.08):
            cand.append(Arm(family, 'sp_max=%.2f' % sm, {'spacing_max': sm}))
    elif family == 'gearing':
        for g in (2.4, 2.9, 3.4, 3.9, 4.4):
            cand.append(Arm(family, 'gearing=%.1f' % g, {'gearing': g}))
    else:
        raise ValueError('未知参数族: %s' % family)
    arms, seen = [Arm(family, 'BASE(现值)')], [b]
    for a in cand:
        p = a.params()
        if any(p == q for q in seen):      # 与基线/前臂等价 → 丢（含 Pass 2 换基线后的重合点）
            continue
        seen.append(p)
        a.overrides = {k: v for k, v in a.overrides.items() if v != b[k]}   # 相对新基线的净覆盖
        arms.append(a)
    return arms
