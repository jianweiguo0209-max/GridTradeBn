...
# ---- 外推闸门(2026-07-26 二版:成交额口径重标定,prod 131 个已关闭格)----
# ⚠ 一版(FILL_OK/DOUBT = 8/18)作废:它拿实盘 **trade 笔数** 3.49/18 当上限,而那是被
# 拆单污染的量(一个挂单被多对手方分批吃 = 多行,拆单比 1.486)。按成交额守恒口径重标定:
#     fill_rate = 实盘成交额/回测理论 = 1.004,逐格 q25=median=q75=1.000,
#     7 个 spacing 箱全在 0.956~1.094,pearson=+0.051 ⇒ **引擎无罪,无间距依赖**。
# ⇒ 不存在"回测高估成交"这个可校正量,闸门性质是**外推闸门**(超出可验证范围)而非高估修正。
#
# 真正被标定住的轴不是间距,是「穿越/bar 密度」:密度≪1 时一根 bar 内至多穿一条线,任何单调
# 路径都数得一样 ⇒ trans_candle_to_tick 的 4-tick 近似**不参与计数**(故 1.004 未验证它);
# 密度>1 时计数**完全由该近似决定**,而那一段零验证。
#   实盘覆盖:median 0.0042/bar   q90 0.0154   max 0.0655
# 本扫描 period=12h ⇒ 720 根 1m bar,换算成 n_fills 阈值如下。
# ⚠ 仅对**全程持有**的臂精确(P1R 关全部主动止损 ⇒ 恒满 720);带止损的臂提前平仓 ⇒ 实际
#   bar 数 <720 ⇒ nf/720 **低估**密度、闸门偏松。几何选点走 P1R,故选点处精确。
_BARS_12H = 720.0
LIVE_DENS_Q90, LIVE_DENS_MAX = 0.0154, 0.0655        # 穿越/bar,实盘 q90 / max
FILL_OK = LIVE_DENS_Q90 * _BARS_12H                  # ≈11.1
FILL_DOUBT = LIVE_DENS_MAX * _BARS_12H               # ≈47.2
