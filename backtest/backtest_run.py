...
def _funding_missing(funding_df, bars_df):
    """持仓窗口内是否缺资金费观测。True = 引擎对该网格按 fr_fee=0 计、资金费止损失效，
    结果系统性偏乐观（见 grid_engine.cal_equity_curve）；标记到结果以免被当真值。
    窗口取 bars_df 的 candle_begin_time(UTC) 跨度，与引擎 merge 口径一致。"""
...
def run_backtest(cache, universe, window_start, window_end, strategy_config,
    ...
    for rt, offset, row in grids:
        ...
        if engine == 'engine':
            ...
            funding_missing = _funding_missing(funding_df, bars_df)  # 窗口内无费率→fr_fee=0、偏乐观，标记之
            ...
            funding_missing = False  # grid_sim 原型本就不建模资金费，非数据缺失
        ...
        results.append({
            ...
            'terminated': bool(terminated), 'funding_missing': bool(funding_missing),
        })
