...
- **仓位参数体系换代（2026-07-07，待部署）**：`leverage×max_rate` 纯冗余对（唯一消费点=乘积、`set_leverage` 零调用）合并为 **`GRID_GEARING`**(3.4，单格挂单总名义额=gearing×cap)；新增 **`ACCOUNT_LEVERAGE`**(最坏净敞口倍数，部署值 3.5)，`CAP_EQUITY_FRAC` 退役降为推导值 `frac=AL/(N×gearing/2)`=0.1716(12 仓)。旧键 `LEVERAGE`/`CAP_EQUITY_FRAC` 设置即 boot RuntimeError（语义变更禁静默映射——**部署前务必 `fly secrets list` 确认两 app 无此二键**）。**无 DB 迁移**：`grids.leverage` 列行为惰性（restore 用持久化 order_num/配置重算、图表只用杠杆无关价格档），专门矩阵 tests/execution/test_gearing_db_impact.py 五用例实证；新行存 gearing。风险预算依据（HL 保证金实测：totalMarginUsed 只计净持仓、挂单预留~1.6%名义额）：AL=3.5→最坏敞口 3.5×equity、账户级回测串联 10 月 +38.7%/最差窗已实现 MDD −3.87%（数据 data/tiercmp/pv_frac_equity.csv）。spec/plan `docs/superpowers/{specs,plans}/2026-07-07-account-leverage-gearing*`。效果：cap testnet $98→$169、mainnet $304→$521（新格生效，存量格不变）。
- **MarketShockBrake（2026-07-08，待部署）**：组合级急动刹车——scheduler 每整点用本轮已拉 universe 1h K线算票池中位数 4h 收益(PIT,零额外 API),|med|≥4% → 本轮只关不开+暂停 2h(存量格/monitor/止损零改动)。回测 GO(四窗双臂 76 仿真:事件捕获 37/37、Δ收益合计 0、被拦格基线净亏、MDD 全面改善;与 PV+0.005 互补已验证,详 memory shock-brake-verdict + data/tiercmp/sb_*.csv)。暂停状态进程内(信号自持~k 小时重启自愈,约束 pause≤k);SHOCK_THR=0 即停用(零部署回滚)。spec docs/superpowers/specs/2026-07-08-market-shock-brake-design.md。
- **PV 止损换形（2026-07-07，待部署）**：实盘 PV 恢复 legacy 满窗语义（signals 原生 15m×108 根 ≈27h 真滚动基线，取数窗口与开格解耦）+ 默认 `pv_pnl_thr` −0.02→**+0.005**、`pv_n` 233→**100**（mult=3/con2=0 不动）。依据：干净数据+对齐费率四窗回测全正（IS +4.80/OOS +2.97/W1 +1.22/W2 +3.72，诚实均值 +2.64%），全胜漂移语义与线上现参（−2.67%）；**语义与门槛必须绑定翻转**（legacy 语义×−0.02 W1 更差）。本质是策略换形：~70% 格首个真量能尖峰即撤（每格均值≈0），盈利引擎=活到期格+回撤止盈。已知缺口：回测管线默认仍漂移语义（研究复现走 data/tiercmp/pv_legacy_sem.py）。spec/plan: `docs/superpowers/{specs,plans}/2026-07-07-pv-legacy-semantics-live*`。上线：main→testnet（观察协议=无报错，顺带核对 pv 退出占比 ~70% 量级）→production，各步需用户批。
