...
---
## 追加待验臂（2026-07-26，消融格过预声明门：t≥1.5 且 ≥RSP111 且无窗<−10bp）

新选币器三个（候选集/注入路径与 RSP111 同口径，全部等权名次和、top-1、冻结禁调）：

| 选币器 | 定义 | 探针六窗合并(预期收缩) |
|---|---|---|
| **D_ESP** | rank(Er_2↑)+rank(Sgcz_5↑)+rank(p12↓) | +10.9bp / t3.20 |
| **D_REP** | rank(Reg_v2_5↑)+rank(Er_2↑)+rank(p12↓) | +10.5 / t3.11 |
| **EP2** | rank(Er_2↑)+rank(p12↓) 双因子裸核 | +9.8 / t2.63 |

- 各配两链：**s030 现役链**（隔离选币器贡献）与 **v2固3**（RSP 家族链谱赢家）→ 追加 6 臂。
- 机制注记：Er_2（围绕 EMA 的均衡振荡度）是比 Reg/Sgcz 更强的燃料否决器——"振荡门控燃料"
  = 消融发现的承重结构；EP2 检验第三因子是否冗余。
- 多臂折扣声明：本周探针累计 ~57 臂，t3.2 为其中最大值，选择效应在场；全样本+HOLD-E 照裁。
- 预注册主臂选择规则更新为：**主臂 = 判定段八窗合计 Calmar 最高的候选臂（RSP 系+本节 6 臂
  全体范围内）**；HOLD-E 部署门不变。
- 落败组档案（不跑）：A_RSEP4 +5.4(HOLD-B −10.9)、B_RSE +3.1(W2 −22.7)、C_RSPD +3.9。

---
## 修订二（2026-07-26）：RSP111 全样本验证失败，战役范围收缩

- **RSP111×五链全部出局**（全样本判定段未过，勿再跑）——这是 stride-5 探针乐观偏差的
  **第 4 次全样本实证**（前三次：W1/W2 St5 翻转、F99 死窗翻正被反证、探针 t2.18→全样本亡）。
- **剩余待验臂（6+锚）**：D_ESP / D_REP / EP2 × {s030 现役链, v2固3}。
  同源折扣加重声明：三者的提名读数（t2.6-3.2）出自同一台探针机器，预期显著收缩。
- 主臂选择规则相应收缩：主臂 = 判定段八窗合计 Calmar 最高的**本节 6 臂之一**。
- **处女地保护条款**：若判定段 6 臂中无任何臂八窗合计 Calmar > 锚臂，**终止，不进 HOLD-E**
  ——最后一块未触碰时段不为将死候选消费。
- 其余流程/判据/红线不变。

---
## 修订三（2026-07-26）：链谱扩为 v2 全族

- 待验臂 = {D_ESP, D_REP, EP2} × {s030, **v2固3, St4, St5, F30, F99**} = **18 臂 + 锚**。
  链参数全部沿用 §2 冻结表（v2 族共性:trailing k0.15/floor1%、pv mult5/n100/thr−1%;
  固损与费率阈按各链;s030=生产现值全套）。
- F99 解禁进场但**保留 carry 标签**：其超额若再次集中于深负费率窗且 HOLD-C 型复苏窗回吐,
  按 carry 定性读数,不按选币边读数。
- 每选币器 rank preload 一次、六链共享(战役引擎阶段 ~1min/臂/窗,增量成本低)。
- 主臂选择规则、处女地保护条款、HOLD-E 部署门不变(范围=本 18 臂)。

---
## 修订四（2026-07-26）：eff 族过门追加

- **eff1**：`p12_eff = cross1/(1+100·mae)`（过去12h实现,标签rt+12h=R平移,同p12注入路径）
  **单因子降序 top-1**——探针 +13.8/t4.91(全项目最强;两死窗全平 W1−1.3/W2+1.4;OOS+44.7)。
  × v2 全族六链 = 6 臂。
- **EPE**：rank(Er_2↑)+rank(p12↓)+rank(p12_eff↓) 等权——+9.2/t2.86。×{s030, v2固3} = 2 臂。
- 落败归档(勿跑): ED2(Er+eff 双重门绞杀 −0.5)、EDP(drift名次第三次稀释失败 +2.3)。
- 待验总计 = 18(修订三) + 8 = **26 臂 + 锚**。主臂选择规则/处女地保护/部署门范围同步扩至 26 臂。
- 折扣备案: 累计~70探针臂;stride-5乐观已四证;eff1 的 t4.91 超多臂噪声期望上限(~3.0)
  为唯一例外读数,仍以全样本+HOLD-E 为准。
