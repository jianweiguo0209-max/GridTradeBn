...
| 臂 | 六窗合并 alpha/t | 地位 |
|---|---|---|
| **RSP111×v2固3** | **+9.4bp / t2.18** | **主臂（预注册唯一裁决对象）** |
| RSP111×St5 | +9.0 / 1.78 | 副臂（同报不裁决） |
| RSP111×F30 | +8.6 / 1.61 | 副臂 |
| RSP111×s030 现役链 | +8.1 / 1.59 | 副臂（分离选币/链贡献） |
| RSP111×St4 | +7.6 / 1.58 | 副臂 |
...
2. **判定段（全轮八窗）**：锚复现 → 六臂 × {W1,W2,OOS,IS,HOLD-A,B,C,D} 全轮。
   汇报：各臂各窗 ret/MDD/Calmar/退出构成 + **主臂八窗合并逐格 alpha t**（5× 样本对
   探针 t2.18 的直接检验；若合并 t<1.5 直接终止,不进 HOLD-E,省一发）。
3. **预注册**（判定段报完、HOLD-E 数据构建**之前**写死并 commit）：建议模板——
   主判据（两条都过才算活）:
   (A) 知识门：主臂八窗全轮合并逐格 alpha **t≥2**；
   (B) 部署门：HOLD-E 上主臂 **Calmar ≥ 锚臂 且 MDD ≤ 锚臂×1.3 且 ret > 锚臂**。
   副臂只记录。判据由你落笔定稿,写完不许挪。
