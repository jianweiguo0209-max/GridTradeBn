...
def test_open_baseline_no_faults(store):
    fake, gx = build_stack(store)
    gid = gx.open('fake', SYM, GP)
    assert gx.grids.get(gid).status == 'ACTIVE'
    assert len(fake.fetch_open_orders(SYM)) == 9


def test_open_transient_timeout_still_reaches_active(store):
    # 前两次挂单请求未达交易所即超时 → ResilientAdapter 重试 → 仍开齐
    fake, gx = build_stack(store, {'create_limit_order':
...
def test_open_lost_ack_no_duplicate_order(store):
    # 第一笔挂单：内层已建单但 ack 丢失 → 重试再发 → 不得产生第二个挂单
    fake, gx = build_stack(store, {'create_limit_order': [RaiseAfter(ccxt.RequestTimeout('lost-ack'))]})
    gid = gx.open('fake', SYM, GP)
    assert gx.grids.get(gid).status == 'ACTIVE'
